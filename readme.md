...
```
Setup:    30-40s ← import torch + HIP 初始化（后台重叠，被 I/O 隐藏）
I/O:      31-49s ← 读 3060 个 .nc 文件 (Lustre 共享存储，节点间波动)
Compute:  13-15s ← 4 张 DCU Z100 并行，topk P90
Save:      1-2s ← 写 92 个 NetCDF
Total:    48-66s (0.8-1.1 min)（取决于节点 Lustre 缓存）
```

Setup 在后台线程与 I/O 并行重叠，wall time = max(Setup, I/O)。小计 = I/O + Compute + Save ≈ 46-66s。

冷启动最佳节点（f16r4n02）：I/O 31s，总时间 48s。其余节点 57-66s。
...
| **+ float16 + 节点优选** | **13s** | **~1.0min** | **~60x** |

## 下一步

- [x] 所有优化完成，冷启动总时间 ~57s（最佳节点 48s）
