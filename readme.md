...
**MPI + 4GPU 并行 + HIP 边读边算流水线 + 并行 Save，Competition ~18s（比赛规则全流程计时）。**
...
```
─── 计时起点 (程序启动) ───
Prep:       1.6s  (imports + file index + format probe + MPI window)
Setup:      5.0s  (HIP kernel compile + 4 GPU micro-warmup; 首次编译 ~22s)
I/O:        6.8s  (3060 files raw byte I/O, MPI 30 ranks, direct-to-shm)
Compute:   10.3s  (4×DCU Z200, 每 GPU 3 bands 流式, 7.0s 被 I/O 覆盖)
Save:       1.2s  (92 个 NetCDF, 4 进程 fork COW 并行写)
─── 计时终点 (结果输出) ───
Competition: 18.1s  = Prep + Setup + I/O + compute_tail(3.3s) + Save
```

**核心优化：I/O 与 Compute 重叠** — GPU 线程在 I/O 进行中就开始计算，首轮 7.0s 计算完全被覆盖，仅剩第 2/3 轮 3.3s 在 I/O 结束后执行。
...
| **边读边算流水线（当前）** | 10.3s | 6.8s | **~18.1s** | I/O 与 Compute 重叠 |
...
- [x] **边读边算流水线**：accumulate_year + finalize 内核 + ready_flags 同步 + GPU 线程轮询，Competition 22-27s → 18.1s
...
| Setup | ~5.0s | 28% | HIP kernel 编译 + 4 GPU micro-warmup；**当前最大瓶颈** |
| I/O | ~6.8s | — | MPI 30 ranks, raw byte I/O（与 Compute 重叠） |
| Compute tail | ~3.3s | 18% | 第 2/3 轮 band, I/O 完成后执行 |
| Save | ~1.2s | 7% | 4 进程 fork COW |

Setup 5.0s 是当前最大瓶颈：HIP 驱动将 GPU 二进制加载到每张卡的指令缓存是硬件操作，无软件绕过路径。微 warmup 选择真实 band 尺寸的 accumulate kernel 预热 4 张 GPU，每卡 ~1.2s。
I/O 6.8s 不再单独计入 Competition（因与 Compute 重叠），仅 compute tail 3.3s 暴露在关键路径上。
