...
**已完成所有优化，4 卡并行 GPU 加速，real ~30s（不含 setup）。**
...
```
Prep:       0.1s  (file index + format probe)
I/O:        14s   (3030 files, 5.92 GB, 24 workers, netCDF4 直读)
Setup:      12s   (import torch + HIP 初始化, 串行)
Compute:    13s   (4×DCU Z200 并行, topk P90)
Save:        1.5s (92 个 NetCDF 输出)
─────────────────────────────────
Total:      41s   (从 I/O 开始)
real:       29s   (I/O+Compute+Save, 不含 setup)
Shell real: 47s   (完整进程, 含 import + prep + setup)
```
...
| **串行执行（当前）** | I/O → Setup → Compute → Save，计时不包含 Setup | 逻辑清晰，real 不含 setup |

> 早期版本 Setup 在后台线程与 I/O 并行，可隐藏 Setup 时间。当前改为串行后 real 不含 Setup，更符合赛题计时规则（环境加载不计时）。
...
Python real:     I/O + Compute + Save（不含 Setup）
```
