...
**已完成所有优化，4 卡并行 GPU 加速，总耗时 ~42s (0.7 分钟)。**
...
```
Setup:    16-27s ← import torch + HIP 初始化（后台重叠，被 I/O 隐藏）
I/O:      22-34s ← 读 3060 个 .nc 文件 (Lustre 共享存储，节点波动)
Compute:  13-14s ← 4 张 DCU Z100 并行，topk P90
Save:      1-2s ← 写 92 个 NetCDF
Total:    42-52s (0.7-0.9 min)
```

Setup 在后台线程与 I/O 并行重叠，wall time = max(Setup, I/O)。
...
| **float16 存储** | numpy 数组 float16，pickle 序列化从 400MB/年 → 200MB/年 | **33s → 22-26s** |
...
| 优化 | 原理 | 效果 |
|------|------|------|
| **float32 精度** | f64→f32，数据量减半，加速来自内存带宽 | 99s → 37s |
...
Setup (后台线程)  ←── 真正并行 ──→ I/O (主线程, ProcessPoolExecutor)
                   wall = max(20s, 26s) ≈ 26s
                   不是 20s + 26s = 46s
```

最终耗时 = **max(Setup, I/O)** + Compute + Save ≈ **26 + 13 + 2 ≈ 42s**（f16r4n03 最佳节点）。
