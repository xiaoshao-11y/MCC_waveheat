...
**MPI + 4GPU 并行 + HIP 融合内核 + GPU 预热 + 并行 Save，Total ~12.0-13.5s（节点和 Lustre 负载波动）。**
...
| GPU 预热 | ✓ dummy kernel 消除首 band JIT，Compute 7.4→5.5s |
...
|------|-----|---------|------|-------|
| f16r4n12/n13（优质） | ~5.5s | ~5.5s | ~1.0s | **~12.0s** |
| f16r4n03/n14（一般） | ~7.0s | ~5.5s | ~1.0s | **~13.5s** |
...
Compute:    ~5.5s  (4×DCU Z200, HIP fused kernel, float16, GPU 预热)
Save:       1.0s  (92 个 NetCDF, 4 进程 fork COW 并行写)
─── 计时终点 ───
Total:      12.0-13.5s (I/O → save end)
Grand:      ~18s (setup + I/O + compute + save)
...
| **+ GPU warmup + parallel save + IO_THREADS=2** | **5.5s** | **5.5-7.0s** | **~12.0-13.5s** | **~140x** |
...
- [x] GPU 预热（dummy kernel 消除 HIP JIT 开销，Compute 7.4→5.5s）
- [x] 并行 Save（multiprocessing fork COW，Save 1.5→1.0s）
- [x] 参数扫描（IO_THREADS=2 最优，SAVE_WORKERS=4）

## 当前状态

Total ~12.0-13.5s（优质节点 ~5.5 + 5.5 + 1.0，一般节点 I/O 波动 +1.5s）。Lustre MDS 寻址为最终瓶颈。

### 瓶颈分析

| 阶段 | 时间 | 占比 | 备注 |
|------|------|------|------|
| I/O | 5.5-7.0s | ~55% | 主要瓶颈，Lustre MDS 寻址 ~4.6s（3060 次 pread），不可优化 |
| Compute | ~5.5s | ~40% | 已充分优化（HIP kernel + warmup），首 band 2.4s / 后续 1.4s |
| Save | ~1.0s | ~8% | 已并行优化 |
...
| 方案 | 结果 |
|------|------|
| 双服务器（split lat range） | I/O 不省（每台仍做 3060 次 pread），wall clock 不变 |
| I/O 与 compute 流水线 | Lustre per-pread 开销 ~1.5ms，分阶段反而多付寻址成本 |
| inode 排序 | stat 操作 MDS 开销远超 seek 节省（见 Problem 15） |
