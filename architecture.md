...
```
sdp/
├── compute_server.py          # 主计算程序 (float32, 4卡 GPU 线程并行)
...
│   └── Climatology/           # float32 输出 (92个 .nc 文件)
├── logs/                      # SLURM 任务日志
...
采用 **float32** 单精度。DCU Z100 (CDNA2) 的 FP64:FP32 算力比为 1:1，float32 加速全部来自内存带宽减半和数据量减半。RMSE 0.000001 °C，精度满足赛题要求。
...
原始数据文件 (3060 个 .nc, 每年 102 天 × 30 年)
  ...
  └────────────────────────────────────┘
       │
       ▼  data: (30, 102, 721, 1440) float32 = 11.84 GB
       │
  ┌────────────────────────┐
...
| 阶段 | 时间 | 占比 | 瓶颈 |
|------|------|------|------|
| Setup | 16-27s | ~40% | import torch + HIP 初始化（驱动硬开销） |
...
脚本自动运行 `sinfo` 查找 idle 节点，用 `--nodelist` 精准指定，避开已分配节点。
